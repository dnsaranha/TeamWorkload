...
-- Create projects table for workload management (simplified version)
...
-- Create tasks table for workload management (simplified version)
CREATE TABLE IF NOT EXISTS workload_tasks (
  ...
  project_id UUID REFERENCES workload_projects(id) ON DELETE CASCADE,
...
alter publication supabase_realtime add table workload_projects;
alter publication supabase_realtime add table workload_tasks;
...
INSERT INTO workload_projects (name, description, start_date, end_date) VALUES
...
INSERT INTO workload_tasks (name, description, estimated_time, start_date, end_date, project_id, assigned_employee_id)
SELECT 
  ...
  p.id,
  e.id
FROM workload_projects p, employees e
...
INSERT INTO workload_tasks (name, description, estimated_time, start_date, end_date, project_id)
SELECT 
  'Database Migration',
